...
import loyer.db.F591RecordDataTools;
import loyer.db.F591RecordDataTools.F591RecordData;
import loyer.db.F591TestDataTools;
import loyer.db.F591ViceDataTools;
import loyer.db.F591ViceDataTools.F591ViceData;
...
public class F591DataView extends LoyerFrame {
  ...
  public static void getView() {
    ...
    EventQueue.invokeLater(new Runnable() {
      ...
      @Override
      public void run() {
        F591DataView win = new F591DataView();
  ...
  public JTable getTestTable() {
    ...
    // 创建行对象
    rowNum = new Vector<>();
    List<F591ViceData> tableList = F591ViceDataTools.getAllByDB(); // 从数据库中获取c211表的内容

    for (Iterator<F591ViceData> i = tableList.iterator(); i.hasNext();) {
      F591ViceData rd = i.next();
  ...
  public void initCountAndPieChart() {
    F591RecordData rd = F591RecordDataTools.getByDate(LocalDate.now().toString());
    if (rd != null) {
  ...
  public void initCOM2() {
    if(portList.contains("COM2") && COM2 == null) {
      ...
      try {
        ...
        //发送副驾测试指令
        SerialPortTools.writeBytes(COM2, new byte[]{(byte) 0xf3, (byte) 0xf4, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0a});
      } catch (TooManyListeners e) {
        JOptionPane.showMessageDialog(null, "COM2:" + e.toString());
      }
    }
  ...
  public void COM1DatasArrived() {
    ...
    F591TestDataTools.insert(datas);//*/
