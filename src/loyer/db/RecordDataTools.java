...
public class RecordDataTools {

  private RecordDataTools() {} //不允许其他类创建本类实例
  /**
   * 通过日期获取表中记录的数据
   * @param date
   * @return
   */
  public static RecordData getByDate(String date) {
    RecordData data = null;
    String sql = "select * from recordtd where recordtime='"+date+"'";
  ...
  public static int insert(String[] datas) {
    if(datas == null || datas.length != 6) {
      ...
      String sql = "insert into recordtd values(?, ?, ?, ?, ?, ?)";
  ...
  public static int update(String[] datas) {
    if(datas == null || datas.length != 6) {
      ...
      String sql = "update recordtd set recordsum='"+datas[1]+"', recordok='"+datas[2]+"', recordng='"+datas[3]+"',"
