...
public class TestDataTools {
  ...
  public static List<TestData> getAllByDB() {
    ...
    String sql = "select * from f519record";
    ...
    try {
      while(rs.next()) {
        ...
        list.add(new TestData(num, action, items, upper, lower, value, unit, result, date));
      }
   ...
   * @return
   */
  public static List<TestData> getByDate(String date) {
    List<TestData> list = new ArrayList<>();
    String sql = "select * from f519record where date='"+date+"'";
    ResultSet rs = DBHelper.search(sql, null);
    try {
      while(rs.next()) {
        int num = rs.getInt(1);
  ...
  public static int insert(String[] datas) {
    if(datas == null || datas.length != 8) {
      ...
      String sql = "insert into f519record(action, items, upper, lower, value, unit, result, date) values(?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  public static void outExcl() {
    ...
    try {
      ...
      //创建可写入的Excel工作簿
      String fileName = "正驾测试数据" + LocalDate.now()+".xls";
      ...
      //查询数据库中所有的数据
      List<TestData> list = getByDate(LocalDate.now().toString());
  ...
  public static class TestData {
    ...
    public TestData(int num, String action, String items, String upper, String lower, String value, String unit,
